...
async function getPlayer(msg, { requireSameVoice = true } = {}) {
  const player = music.getPlayer(msg.guild.id);
  if (!player) {
    await temp(msg, { embeds: [errorEmbed(`Chưa có player. Dùng ${config.prefix}play trước.`)] });
    return null;
  }
...
async function onCommand(msg) {
  ...
  try {
    ...
    if (command("help", cmd)) {
      return temp(msg, { embeds: [helpEmbed(config.prefix, config.commands)] }, 12000);
    }
    ...
    if (command("pause", cmd)) {
      ...
      if (!Number.isInteger(volume) || volume < 1 || volume > 100) {
        return temp(msg, { embeds: [errorEmbed(`Dùng ${config.prefix}volume 1-100`)] });
      }
      await player.setVolume(volume);
    } else if (command("seek", cmd)) {
      const seconds = parseTime(arg);
      if (seconds === null || seconds < 0) {
        return temp(msg, { embeds: [errorEmbed(`Dùng ${config.prefix}seek 1:30`)] });
      }
      await player.seek(seconds * 1000);
    } else if (command("loop", cmd)) {
      const mode = arg.toLowerCase() === "off" ? "none" : arg.toLowerCase();
      if (!["none", "track", "queue"].includes(mode)) {
        return temp(msg, { embeds: [errorEmbed(`Dùng ${config.prefix}loop off|track|queue`)] });
      }
      ...
      if (!Number.isInteger(number) || number < 1 || number > list.length) {
        return temp(msg, { embeds: [errorEmbed(`Dùng ${config.prefix}remove <số>`)] });
      }
      ...
      if (!["on", "off"].includes(mode)) {
        return temp(msg, { embeds: [errorEmbed(`Dùng ${config.prefix}autoplay on|off`)] });
      }
